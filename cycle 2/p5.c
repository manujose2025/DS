	...
	else
	{
		...
		else
		{
			...
			while(temp != start)
			{
				if(count == pos)
				{
					temp1->next = temp->next;       //temp->next = start;
					temp->next = NULL;
...
void search(int element)
{
	...
	else if(start->next == start)
    {
        if(start->data == element)
			{
				printf("The element %d is present in the list at position 1.\n", start->data);
				return;
			}
    }
	{
		...
		{
			printf("The element %d is present in the list at position %d.\n", start->data, pos);
			return;
		}
...
void main()
{
	...
	{	
		...
		printf("Enter the Circular Linked List operation that you would like to perform : ");
		scanf("%d", &option);
		switch(option)
		{
			case 1:
